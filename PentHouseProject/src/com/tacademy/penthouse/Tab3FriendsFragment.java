...
public class Tab3FriendsFragment extends Fragment {
	...
	ExpandableListView fListView;
	UserRoomAdapter fAdapter;
	String[] t = {"aa","bb"};
	...
	public View onCreateView(LayoutInflater inflater,
		...
		fListView = (ExpandableListView)v.findViewById(R.id.friends_list);
		fAdapter = new UserRoomAdapter(getActivity());
		
		fAdapter.setOnAdapterItemClickListener(new UserRoomAdapter.OnAdapterItemClickListener() {
		...
		initData();
		for(int i=0; i<fAdapter.getGroupCount(); i++){
			fListView.expandGroup(i);
		}
		fListView.setOnGroupCollapseListener(new OnGroupCollapseListener() {
			
			@Override
			public void onGroupCollapse(int groupPosition) {
				fListView.expandGroup(groupPosition);
				
			}
		});
		
		fListView.setOnGroupClickListener(new ExpandableListView.OnGroupClickListener() {
	...
	private void initData(){
		for(int i = 0; i < rData.length; i++){
			...
			while (j < iData.length){
				...
				if(iData.length - j != 1){
					fAdapter.put( rData[i], iData[j],iData[j+1] );
					if(iData.length - j != 1){
						j = j + 2;
					}else{
						j++;
					...
					fAdapter.put( rData[i], iData[j],null);
					j++;
				}				
			}
		}
