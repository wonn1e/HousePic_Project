...
public class ItemInfoActivity extends FragmentActivity {
	...
	ItemData iData;
	ItemLikeShowListDialog item_like_dialog;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		item_like_dialog = new ItemLikeShowListDialog();
		...
		item_like.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				Fragment f = getSupportFragmentManager().findFragmentByTag(ITEMLIKE_TAG);
				if(f == null){
					FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
					ft.replace(R.id.container, item_like_dialog);
					ft.commit();
				}
				
				Bundle b = new Bundle();
				b.putParcelable("iData", iData);
				b.putParcelableArray("rData", rData);
			}
		});
